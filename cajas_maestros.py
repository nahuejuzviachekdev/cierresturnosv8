"""
Proceso 030 - Sincronizacion de tabla maestra: Cajas.
Depende de estaciones (sincronizar estaciones primero).
Extrae la tabla directamente de cada base SQL Server (solo lectura)
y la replica en el schema PostgreSQL correspondiente.
Uso: python cajas_maestros.py
"""
import os
import re
import sys
...
# Estaciones primero (Cajas depende de Estaciones via FK)
TABLAS = [
    {'proceso': '027_Estaciones', 'sql_table': 'Estaciones', 'pg_table': 'estaciones'},
    {'proceso': '030_Cajas',      'sql_table': 'Cajas',      'pg_table': 'cajas'},
]
...
def main():
    ...
    log_path  = os.path.join(LOGS_EXEC_DIR, f'cajas_maestros_{stamp}.txt')
    logger    = Logger(log_path)
    pg_tunnel.configurar_logging(logger.log)

    logger.log('=' * 60)
    logger.log('Cajas Maestros -- Proceso 030 (incluye Estaciones)')
